...
public class DailyCategoryAdapter extends RecyclerView.Adapter<DailyCategoryAdapter.ItemViewHolder> {
    private ShowDailyFragment showDailyFragment;
    private String label;
    private List<DailyThings> dailyThingsList;

    public DailyCategoryAdapter(ShowDailyFragment showDailyFragment, String label) {
        this.showDailyFragment = showDailyFragment;
        this.label = label;
        this.dailyThingsList = GlobalManager.dailyMap.get(label);
    }
    ...
    public void onBindViewHolder(@NonNull final ItemViewHolder holder, final int position) {
        ...
        final String labelPos=dailyThings.getIdentity();//"label:pos"
    ...
    private void deleteItem(String pos,int position){//label:pos
//        //删除最近添加
//        for (DailyThings item:
//             GlobalManager.dailyThingsList) {
//            if(item.getIdentity().equals(pos)){
//                GlobalManager.dailyThingsList.remove(item);
//            }
//        }

        //删除全局缓存
        dailyThingsList.remove(position);
